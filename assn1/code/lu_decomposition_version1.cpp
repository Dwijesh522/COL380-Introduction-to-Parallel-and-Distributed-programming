...
pair<int, float> get_maximum_element_index(vector<vector<float>* >& mat, int c, int r1, int r2);
void print_error(vector<vector<float>*> &mat_dup, vector<int> &permutation, vector<vector<float>*> &lower, vector<vector<float>*> &upper);
...
int main(int argc, char *argv[])
{
	...
	vector<vector<float>* > mat, upper, lower, mat_dup;
	...
	// ------------------------------------------------------------------------------------------
	// ------------- sequential timing	: 3.5 sec (two dimensional matrix pointing to floats)
	// ------------- parallel timing	: 2.9 sec (two dimensional matrix pointing to floats)
	// ------------------------------------------------------------------------------------------
	...
	#pragma omp parallel num_threads(NUM_THREADS)
	{
		#pragma omp sections
		{
			#pragma omp section
			{
				for(int i=0; i<MATRIX_SIZE; i++)
				{
					...
					mat_dup.push_back(new vector<float>(MATRIX_SIZE, float(0)));
				}
			}
			#pragma omp section
			{
		...
		for(int i=0; i<MATRIX_SIZE; i++)
		{
			...
			{
				float random_num = float(drand48());
				(*mat[i])[j] = random_num;
	...
	for(int k=0; k< MATRIX_SIZE; k++)
	{
		...
		vector<float>* temp_row_pointer = mat[k];
		mat[k] = mat[k_dash];
		...
		auto start = (*lower[k]).cbegin();
		auto end = (*lower[k]).cbegin()+k;
		vector<float> temp_sub_vector_lower_matrix(start, end);
...
// ------------------------------------------------------------------------------------------------------
// ------------- sequential time:	4.9 seconds (two dimensional matrix pointing to floats)
// ------------- parallel time:		4.9 seconds (two dinensional matrix pointing to floats)
// ------------- parallel time: 	2.2 seconds (one dimensional vector pointing to vector of floats)
// ------------------------------------------------------------------------------------------------------
pair<int, float> get_maximum_element_index(vector<vector<float>* > &mat, int c, int r1, int r2)
{
	float max_element=0;
